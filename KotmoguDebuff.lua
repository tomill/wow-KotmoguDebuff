...
hooksecurefunc("CompactUnitFrame_UpdateDebuffs", function(frame)
    if (not UnitIsPlayer(frame.displayedUnit) or not frame.optionTable.displayDebuffs) then
        if frame.KotmoguDebuff then
            frame.KotmoguDebuff:Hide()
        end
        return
    ...
    -- if unit has orb
    local _, _, _, _, _, _, _, _, _, spellId, _, _, _, _, _, _, val2 = AuraUtil.FindAuraByName("Orb of Power", frame.displayedUnit, 'HARMFUL')

    --    spellId, val2 = 121176, 120 -- debug
    ...
    -- set size
    local baseSize = frame:GetHeight() * 0.7
    if baseSize <= 0 then
        return
    else
    ...
    -- display
    orbframe.debuff:SetTextColor(1, 1, 1, 1)
